...
/// The SVG file opened in a viewer: one per user, in the runtime directory where the platform has
/// one and the temporary directory otherwise.
...
///
/// Collect graph fields in the header filter and skip every body. The iterator therefore yields
/// only read errors.
