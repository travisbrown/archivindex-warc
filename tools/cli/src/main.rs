...
enum Command {
    ...
    ///
    /// For compatibility with tools that hash chunk framing, recompute eligible HTTP request and
    /// response payload digests over the stored message body. Keep the declared algorithm,
    /// encoding, and label. Skip segmented or truncated records and warn when a digest or body
    /// cannot be processed. This does not guarantee that another validator accepts the file.
    ...
    Lint {
        ...
        /// number on standard input, selects gzip decompression, whose member framing is checked as
        /// well.
        #[arg(short, long, value_name = "FILE", value_hint = clap::ValueHint::FilePath)]
        input: PathBuf,
    ...
    /// with a warning.
    LoadRevisitIndex {
        /// The WARC file to index, or - for standard input, or a directory whose .warc and .warc.gz
        /// files are indexed in file-name order.
    ...
    /// WARC-Refers-To names, when that response is in the file. The rest of its capture is removed
    /// with it: every record WARC-Concurrent-To links to the revisit, in either direction and
    /// through any number of records. Every other record is copied as read.
...
enum RewriteTarget {
    ...
    ///
    /// The software field is written as `NAME/VERSION` and the operator field as `NAME <EMAIL>`. A
    /// name given alone keeps the version or email address the record has; a version or email
    /// address given alone replaces the one the record has, and fails when there is none.
