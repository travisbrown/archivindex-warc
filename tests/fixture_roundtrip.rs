...
/// Describe the first difference between the input and the round-tripped output, or `None` if they
/// are byte-identical.
///
/// Archives run to megabytes, so a difference is reported as the offset of the first differing byte
/// plus a short window of context from each side, rather than as a full dump.
fn describe_difference(source: &[u8], written: &[u8]) -> Option<String> {
    ...
    // Without a differing byte, one archive is a prefix of the other, and they first differ where
...
/// Check byte preservation at the raw and grammar layers, semantic equality after rendering, and
/// the expected failures listed in `UNREADABLE`.
fn assert_roundtrip_is_faithful(set: &str, name: &str) {
    ...
    // Compare semantic values after rendering and parsing again.
