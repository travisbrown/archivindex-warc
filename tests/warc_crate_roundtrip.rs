...
/// Describe the first difference between two normalized archives, or `None` if they encode the same
/// records.
///
/// Bodies run to megabytes, so a differing body is reported by length and by the offset of its
/// first differing byte rather than as a full dump.
...
// The fixtures pywb treats as valid archives: the same set the pywb digest and gzip-member tests
// cover. The remaining pywb fixtures are deliberately malformed or non-conforming and have no round
// trip to compare.
roundtrip_tests!(
    "pywb",
...
// The fixtures warcio treats as valid archives. `example-digest.warc` is included because only one
// of its declared digests is wrong: the records themselves are well-formed. The truncated and
// deliberately mis-compressed fixtures are excluded.
roundtrip_tests!(
    "warcio",
