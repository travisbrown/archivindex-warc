// Each integration test binary compiles this module separately and uses only the helpers it needs,
// so items unused by one binary are still live in another.
#![allow(dead_code)]
...
///
/// This is what the round-trip suites compare over: reading the bytes directly keeps a crate's path
/// constructors and gzip backend out of a comparison that is about WARC handling.
...
/// Read every record of an uncompressed archive against the grammars, then write the records back
/// out.
///
/// Apply each field's grammar while retaining its source bytes. A grammar error stops the round
/// trip.
...
/// Read every record of an uncompressed archive, lift it, render it back, and lift the rendering
/// again, requiring the second lift to equal the first.
///
/// Semantic rendering normalizes headers, so this compares parsed values rather than bytes.
///
