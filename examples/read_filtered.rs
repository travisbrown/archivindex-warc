//! Read a gzip-compressed archive, skipping the records whose target URI names one of the files
...
fn main() -> std::io::Result<()> {
    ...
    let mut count = 0;
    let mut skipped = 0;
    // The closure borrows the counter for as long as the iterator lives, which is the loop, so the
    // count can be read again once the loop has ended.
    let kept = file
    ...
    for record in kept {
        let record = record.expect("record is readable");
        count += 1;
        println!(
            "Found record. Data:\n{}",
...
fn has_matching_filename(u: &str, matches: &[String]) -> bool {
    let url = url::Url::parse(u).expect("target URI is not a valid URL");
    url.path_segments()
