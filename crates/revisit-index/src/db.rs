...
///
/// The trait lives in this private module, so [`Connection`] and [`rusqlite::Transaction`] are its
/// only implementors and no caller can name it to add another.
pub trait Handle {
...
///
/// Wait briefly for competing writers instead of immediately returning `SQLITE_BUSY`. Bulk
...
impl Store<Connection> {
    ...
    /// database, and [`OpenError::SchemaVersion`] when the database was written by an incompatible
...
impl<C: Handle> Store<C> {
    ...
    /// [`Error::InvalidDigestLength`] for a digest this index cannot store, [`Error::Database`] for
    /// a query failure, and a `Malformed*` variant for a row this crate did not write.
    ...
    ///
    /// Returns [`Error::UnsupportedDigestAlgorithm`], [`Error::UndecodableDigest`], or
    /// [`Error::InvalidDigestLength`] for a digest this index cannot store, and [`Error::Database`]
    /// for a write failure.
