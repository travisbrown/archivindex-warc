...
impl<C: Handle> Store<C> {
    ...
    /// # Errors
    ///
    /// Returns [`IngestError::MalformedHttpResponse`] or [`IngestError::MalformedWarcPayload`] for
    /// unreadable record metadata, and [`IngestError::Index`] for malformed persisted state or a
    /// SQLite failure.
...
struct HttpMetadata {
    ...
    /// The `Vary` field, naming the request fields that select the representation, with any several
    /// lines the response sent it as already combined by [`declared_vary`].
    vary: Option<String>,
