...
fn payload_less_revisit_never_becomes_the_resource_record() -> Result<(), Box<dyn StdError>> {
    ...
    // WARC 1.1 permits an identical-payload-digest revisit without `WARC-Refers-To`. The revisit's
    // own identity must not be recorded as the resource's representation.
...
///
/// An empty first line would otherwise be the whole field, leaving state that a later request could
/// revalidate against even though the server named a selecting field.
#[test]
...
fn non_200_response_registers_its_payload_but_not_the_resource() -> Result<(), Box<dyn StdError>> {
    ...
    // An error page may still be revisited by digest, but it is not the resource's representation,
    // so its validators must not drive conditional requests.
    assert!(outcome.payload_inserted);
    assert!(!outcome.resource_updated);
    assert_eq!(
        index
