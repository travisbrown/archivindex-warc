...
-> Result<(), Box<dyn std::error::Error>> {
    ...
    // State captured by an earlier crawl that identified itself as a desktop client, for a response
    // that declared its representation selected by `User-Agent`.
    let index = Index::open(&database)?;
    index.insert_payload(&RevisitTarget {
...
/// A challenge cookie is injected per request rather than configured, so a response selected by it
/// is only recorded correctly if the request as sent is what resolves the declared `Vary`.
...
fn a_repeated_discovery_is_an_identical_payload_revisit() -> Result<(), Box<dyn std::error::Error>>
{
    ...
    // The revisit uses the identical-payload-digest profile and points back at the original record,
    // capture URI, and date, sharing the original's payload digest.
...
-> Result<(), Box<dyn std::error::Error>> {
    ...
    // The revisit block is the whole `304` response as received, which is nothing but its head, so
    // it is not truncated.
...
/// Responses to methods other than `GET` or `HEAD` must not supply validators for a later
/// conditional `GET`.
#[test]
...
/// Supplied cookie pairs override stored pairs of the same name. Other stored pairs remain, and all
...
/// A request the server may have already acted on is not replayed, whatever the retry configuration
/// allows, since repeating it would ask for that action a second time.
#[test]
