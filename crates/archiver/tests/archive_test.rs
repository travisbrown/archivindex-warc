...
fn archive_solves_and_retains_sucuri_cookie_challenges() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // The challenge page and the response it guarded are both captured, but answering the challenge
    // is not a redirect.
...
-> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(summary.is_complete());
    assert_eq!(summary.captures.len(), 2);
    // The challenge page, the proof of work, and the response it guarded are all captured, and none
...
-> Result<(), Box<dyn std::error::Error>> {
    ...
    const URLS: usize = 12;
    // The first response stalls until the event sink releases it, which it does once every capture
    ...
    // With the first capture stalled, dispatch stopped at twice the concurrency and resumed once it
    // was recorded.
