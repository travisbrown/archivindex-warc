...
///
/// Defaults to this crate's name and version. A configured software entry must provide both fields.
...
///
/// Block and payload settings override the general algorithm and encoding. An unspecified encoding
/// uses the selected algorithm's recommended format. The default is SHA-256 in Base16. Captured
/// response payloads are digested when transfer decoding succeeds; revisits use the original
/// payload's digest.
///
/// A revisit index matches a response to an earlier capture by payload digest, so captures under a
/// different payload algorithm do not match those an index already holds.
#[derive(Clone, Debug, Eq, PartialEq, serde::Serialize, serde::Deserialize)]
