...
//! [`Crawl`](session::Crawl) driver follows discovered links depth first. Sessions retry transient
//! network failures, archiving the exchanges of every attempt, and can use a persistent revisit
...
///
/// Configuration supports formats such as TOML and JSON. Top-level fields are optional; configured
/// software entries require both name and version. Unknown fields are errors. Durations are
/// `humantime` strings such as `30s` or `10m`. Write `"unbounded"` to remove a default limit. The
/// defaults, as TOML, are:
///
/// ```toml
...
pub struct Config {
    ...
    /// is recorded with a `WARC-Truncated` reason of `time`. Each operation is timed on its own, so
    /// a slow peer that keeps sending never trips it; [`max_capture_time`](Self::max_capture_time)
    ...
    /// [`Config::DEFAULT_MAX_CAPTURE_TIME`].
    ...
    /// Whether to compress each WARC record as an independent gzip member.
    ///
    /// Individual records can be decompressed without reading the rest of the file.
    pub gzip_warc: bool,
    ...
    /// an earlier capture.
    ///
    /// Small duplicates are stored as full responses to avoid the overhead of revisit records.
    /// Conditional `304 Not Modified` responses with a known original still become revisits. Zero
    /// disables the minimum length; the default is [`Config::DEFAULT_MIN_REVISIT_PAYLOAD_LENGTH`].
