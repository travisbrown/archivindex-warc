...
impl CookieJar {
    ...
    /// withholding the rest, in particular the clearance a challenge issued, which the host expects
    /// back on every later request.
    ///
    /// Several supplied lines are combined into the single one a request must send, as [RFC 6265
    /// section 5.4](https://www.rfc-editor.org/rfc/rfc6265#section-5.4) requires, which is also the
    /// value a declared `Vary: Cookie` is then resolved against.
    #[must_use]
    pub fn merged<'a>(
        &self,
        url: &Url,
...
impl Archiver {
    ...
    ///
    /// Recover the jar after lock poisoning. Callers perform only cookie updates while holding the
    /// lock.
