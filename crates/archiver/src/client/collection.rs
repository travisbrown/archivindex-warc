...
pub struct CollectionOptions<'a> {
    ...
    ///
    /// A response declaring `Vary` is stored with this request's values for the fields it names, so
    /// that a later run configured differently does not revalidate against another variant.
...
impl Collection {
    ...
    ///
    /// `request` holds every field the new request will carry, the resolved `Cookie` included, used
    /// to select the same representation variant as the earlier capture.
    pub fn original(
        &self,
    ...
    /// Record every captured hop and add either a page summary or failure. Returns the new capture
    /// summary, or `None` for a recorded failure.
    ///
