...
impl Archiver {
    ...
    ///
    /// The cookie is restricted to the exact host and, for an HTTPS URL, to HTTPS requests. It is
    /// also stored in the WARC request records.
    ///
    /// A host that later answers a recognized challenge adds its clearance to what is held for it,
    /// replacing a cookie of the same name.
    ///
    /// # Errors
    ///
