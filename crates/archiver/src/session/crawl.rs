...
///
/// Initial requests run in order, including duplicates. After each capture, discovered links run
/// depth first in processor order. Discoveries already present among the initial or discovered URLs
/// are skipped unless deduplication is disabled.
pub struct Crawl<'a> {
...
impl<'a> Crawl<'a> {
    ...
    /// Skip a discovered URL that repeats one already given or discovered, or request it again when
    /// `dedupe` is false.
    ///
    /// Without deduplication, the processor is responsible for ending a crawl of pages that link to
    /// each other.
    #[must_use]
