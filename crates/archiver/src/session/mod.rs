...
//! repeated request for a URL whose earlier response carried an `ETag` or `Last-Modified` validator
//! is conditional, so that the server may answer `304 Not Modified` instead of repeating the
...
pub struct Capture<'a> {
    ...
    pub status: u16,
    /// The entity-body after transfer decoding, or stored body bytes if decoding fails. Content
    /// coding is preserved. A revalidated repeat has an empty body; its payload remains in the
    /// earlier capture.
    pub payload: &'a [u8],
...
impl<'a> Capture<'a> {
    ...
    ///
    /// The status and headers are read from `response`, so a driver is shown exactly what a session
    /// would show it for that recording. Returns `None` if `response` does not begin with a
    /// complete HTTP response head.
    #[must_use]
    pub fn new(
        url: &'a str,
        final_url: &'a str,
...
/// outcome of that request to [`inspect`](Self::inspect) or [`failed`](Self::failed) before calling
/// `next` again. A driver keeps whatever state it needs to decide the next request: a queue of
/// discovered links, a pagination cursor, or the results of earlier captures. A request the session
/// was cancelled before completing is reported to neither method.
///
...
pub trait Driver {
    ...
    ///
    /// Called after inspection (or failure) and successful recording, before any `Written` event or
    /// next request, even when cancellation was requested after capture. `Some` describes the
    /// recorded capture, including unexpected truncation via [`CaptureSummary::is_partial`]. `None`
    /// means a capture failure or inspection rejection was recorded. Cancelled attempts and
    /// recording errors receive no acknowledgment.
    ///
    /// Recording does not imply publication: the session can still fail to finalize its archive.
    /// Only a successful [`Session::run`] result confirms publication. Does nothing by default.
    fn recorded(&mut self, capture: Option<&CaptureSummary>) {
        let _ = capture;
    }

    /// Note that the request last returned by [`next`](Self::next) exhausted its capture attempts.
    ///
    /// The session continues with the next request, so a driver waiting on the failed URL must move
    /// on rather than return it again. Does nothing by default.
...
impl<'a> Session<'a> {
    ...
    ///
    /// A name or version holding a control character cannot be written as a `warc-fields` value, so
    /// [`run`](Self::run) fails with [`Error::WarcFields`] before creating any output.
    #[must_use]
