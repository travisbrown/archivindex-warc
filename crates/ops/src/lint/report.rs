...
///
/// A rule added to a pass names and describes what it reports, since this crate knows neither. The
/// name belongs to the rule that reports it and should tell it from the names of the rules this
/// crate defines, which [`Violation::rule`] lists.
...
impl Custom {
    /// Create an error with the given rule name and message.
    #[must_use]
    ...
    /// Create a warning for a convention the file does not follow.
    #[must_use]
...
pub enum Violation {
    ...
    MissingWarcinfoId,
    /// A record's `WARC-Warcinfo-ID` does not name the `warcinfo` record that most closely precedes
    /// it.
    ...
    /// A revisit lacks one or more of `WARC-Refers-To`, `WARC-Refers-To-Target-URI`, and
    /// `WARC-Refers-To-Date`.
    ...
    /// A violation reported by a custom rule.
    #[error("{0}")]
    #[serde(untagged)]
    Custom(Custom),
}
...
/// A lint finding associated with a record or the whole file.
///
/// The serialized form is one flat object: the position and identifier of the record the finding is
/// against, if one is, then the rule's name and whatever the rule reports. A warning names its
...
mod tests {
    ...
    /// A custom finding serializes as the flat object its own rule name tags, and a finding against
    /// no record carries no position.
    #[test]
