...
///
/// Custom rules run after built-in rules, in registration order, for each successfully parsed
/// record. Stateful rules can report file-wide findings in [`finish`](Rule::finish).
///
/// A rule passed to [`with_rule`](super::Linter::with_rule) by mutable reference is borrowed for
/// the life of the pass, so a rule that gathers a summary beside its findings can be read once the
/// pass is done with.
///
/// # Examples
///
...
///                 "no_target_uris",
///                 "no record carries a target URI",
///             ));
///         }
///     }
/// }
...
pub trait Rule {
    ...
    ///
    /// These findings follow every record's result, so one against a record already reported clean
    /// leaves that report as it was.
    fn finish(&mut self, findings: &mut Findings<'_>) {
        let _ = findings;
    }
