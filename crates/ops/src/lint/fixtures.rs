...
/// What a fixture writes where the digest of the field's subject belongs, which rendering replaces
/// with that digest. A test wanting a digest a record does not have writes its own.
...
///
/// These fixtures use unchunked HTTP messages, so their payload begins after the header section.
/// Resource records use the whole block.
...
/// A `response` record made the `revisit` of the record `original` identifies, under the identical
/// payload digest profile.
///
...
///
/// Each finding's rule name is checked against its serialized form here, so every test expecting a
/// finding checks that pairing too.
