...
impl<R: BufRead> Linter<'_, R> {
    ...
    /// closest one, that a `warcinfo` record names its collection and is in the file named for it,
    /// and that a `request` record targets the collection's host.
...
mod tests {
    ...
    /// compressed file is wrong in the uncompressed one it was compressed from, and the other way
    /// about.
    #[test]
    ...
    /// A host is compared without regard to case, so the first capture's differently spelled one is
    /// not a finding.
    #[test]
