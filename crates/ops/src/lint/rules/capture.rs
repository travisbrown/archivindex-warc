...
impl<R: BufRead> Linter<'_, R> {
    ...
    /// Check the capture sequence rule, and that a record outside the response and metadata slots
    /// of a capture names no record in `WARC-Concurrent-To`, and set the expectation for the next
    /// record.
    ...
    /// Check that a record outside the response and metadata slots of a capture names no record in
    /// `WARC-Concurrent-To`.
...
mod tests {
    ...
    /// A `request` linking to its response, a `metadata` record outside a capture, and a record of
    /// another type all name a record where none should.
    #[test]
