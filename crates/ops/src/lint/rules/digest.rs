...
impl<R: BufRead> Linter<'_, R> {
    /// Check that a record declares the digests its block calls for, and that each is the digest of
    /// what it covers.
    ///
...
///
/// The target URI must name HTTP or HTTPS, and the media type must be `application/http` or absent,
/// which is what the record layer accepts when extracting a payload.
