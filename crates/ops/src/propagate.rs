...
//!
//! A revisit stands for the payload of the record its `WARC-Refers-To` names, so it can carry that
//! record's `WARC-Identified-Payload-Type`. Records are copied as read, in the order read; the only
//! change is the field added to a revisit that lacks it.
...
///
/// A revisit without the field receives, as read, the value declared by the response record whose
/// `WARC-Record-ID` its `WARC-Refers-To` names, when that response is in `input` and its value is a
/// media type. The field is placed where the conventional header order puts it among the fields the
/// revisit has. Every other record, and every revisit whose original is not such a response, is
/// copied as read. A path with a `.gz` extension names a gzip-compressed file; a compressed output
/// holds one gzip member per record. A temporary file beside `output` is moved into place after the
/// last record is written.
///
/// # Errors
///
/// Returns an error when the input and output paths are the same, the input is standard input (the
/// operation reads it twice), a file cannot be opened, a record cannot be read or written, or the
/// output cannot be flushed or moved into place.
...
///
/// A value that is not a media type is not repeated, and the first of several responses sharing an
/// identifier is the one whose value is kept.
