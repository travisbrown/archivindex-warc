...
///
/// A revisit is removed when its `WARC-Target-URI` equals, apart from surrounding white space, that
/// of the response record whose `WARC-Record-ID` its `WARC-Refers-To` names, when that response is
/// in `input`. The rest of its capture is removed with it: every record that `WARC-Concurrent-To`
/// links to the revisit, in either direction and through any number of records, such as its request
/// and the metadata naming either. Every other record is copied as read. A path with a `.gz`
/// extension names a gzip-compressed file; a compressed output holds one gzip member per record. A
/// temporary file beside `output` is moved into place after the last record is written.
///
/// # Errors
///
...
///
/// A capture is the records `WARC-Concurrent-To` links to such a revisit, in either direction and
/// through any number of records. The first of several responses sharing an identifier is the one
/// whose target URI counts.
...
mod tests {
    ...
    fn removes_each_revisit_of_its_originals_uri_with_its_capture() {
        let contents = [
            ...
            // A revisit of the original's URI, removed with the request it names and the metadata
            // naming that request.
            record(
                "request",
                "<urn:uuid:3>",
