...
//!
//! Two WARC files are merged into one, with every record of the first file preceding every record
...
/// records are merged and `WARC-Filename` is rewritten as described in the module documentation. A
/// path with a `.gz` extension names a gzip-compressed file; a compressed output holds one gzip
/// member per record. A temporary file beside `output` is moved into place after the last record is
/// written.
///
/// # Errors
///
...
///
/// Field names are matched case-insensitively. `isPartOf` is always allowed to vary, whether or not
/// it appears in `ignored_fields`.
///
/// # Errors
///
...
mod tests {
    ...
    /// dropped record are redirected, and other references and records are untouched.
    #[test]
    fn merges_matching_warcinfo_records() {
        ...
        // The second file's warcinfo record is kept, at the first file's position, under the name
        // of the file it is now in.
        assert_eq!(
    ...
    /// Distinct warcinfo records are retained; their filenames are updated when present.
    #[test]
