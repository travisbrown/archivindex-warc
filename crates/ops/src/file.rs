...
///
/// A path names a gzip file by its extension; standard input is gzip when it begins with the gzip
/// magic number.
...
/// next's, and are numbered from zero across the inputs. A record the closure returns as `None` is
/// dropped. An input is decompressed when its path ends in `.gz`; a compressed output holds one
/// gzip member per record.
///
/// The records are written to `<output>.partial`, synced, and moved into place once the last one is
/// written, so a failure partway through leaves any file already at `output` as it was, and on Unix
/// the destination directory is synced before success is returned. The move replaces the name
