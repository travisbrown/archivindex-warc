...
/// all field values, bodies, and record order are preserved. A path with a `.gz` extension names a
/// gzip-compressed file; a compressed output holds one gzip member per record. A temporary file
...
/// cannot be read or written, or the output cannot be flushed or moved into place.
...
fn canonicalize_header(header: &mut raw::RecordHeader) {
    ...
    // This stable sort keeps repeated fields and extension fields in their original relative order.
    // Extension fields all receive the same rank after every standard field.
    header
        .headers
