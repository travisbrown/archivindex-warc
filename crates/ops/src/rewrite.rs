...
///
/// A value left `None` keeps what the record has. The `software` field is written as `name/version`
/// and the `operator` field as `name <email>`, so a name given alone keeps the version or email
/// address the record already has, and a version or email address given alone replaces the one the
/// record has.
...
impl TwoPartField {
    ...
    ///
    /// A tail given alone replaces the one the value has, and is an error when the value has none.
    fn set(
        &self,
...
///
/// Each supplied field replaces all existing values, or is appended if absent. `Content-Length` is
/// recomputed. An existing block digest is recomputed with its declared algorithm and that
/// algorithm's recommended encoding; unsupported algorithms cause an error. If no digest is
/// declared, SHA-256 is added. Other record types are copied unchanged. A `.gz` path selects gzip;
/// output uses one member per record and is published from `<output>.partial` after writing.
///
/// # Errors
///
/// Returns an error when the input and output paths are the same, a file cannot be opened, a record
/// cannot be read or written, a warcinfo record cannot be rewritten, or the output cannot be
/// flushed or moved into place.
