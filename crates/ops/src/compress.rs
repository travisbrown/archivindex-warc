...
///
/// Each record is written as an independent gzip member at compression level `level`, from 0 (no
/// compression) through 9 (best compression). Records are copied byte for byte; nothing is
/// reordered, respelled, or validated beyond what reading requires. The output is flushed before
/// returning.
///
/// # Errors
///
/// Returns [`Error::Write`] carrying [`write::Error::InvalidGzipCompressionLevel`] when `level` is
/// greater than 9, before anything is read, [`Error::Read`] when a record cannot be read, and
/// [`Error::Write`] when a record cannot be written or the output cannot be flushed.
