...
/// An algorithm together with the encoding its digests are written in.
#[derive(Clone, Copy, Debug, Eq, Hash, PartialEq)]
pub struct Format {
    /// The digest algorithm.
    pub algorithm: Algorithm,
    /// The encoding of the digest value.
    pub encoding: Encoding,
}

impl Format {
    /// An algorithm with the encoding annotation #80 recommends for it.
    #[must_use]
    pub const fn recommended(algorithm: Algorithm) -> Self {
        Self {
            algorithm,
            encoding: algorithm.recommended_encoding(),
        }
    }
}

impl From<Algorithm> for Format {
    /// The algorithm's recommended format.
    fn from(algorithm: Algorithm) -> Self {
        Self::recommended(algorithm)
    }
}
...
/// annotation.
///
/// Both halves retain their original spelling for [`Display`]. Equality compares normalized
/// algorithms and decoded digest bytes. Custom labels ignore ASCII case; undecodable values compare
/// exactly as written. Use [`algorithm_as_read`](Self::algorithm_as_read) and
/// [`value`](Self::value) to compare spellings.
#[derive(Clone, Debug)]
...
impl LabelledDigest {
    ...
    /// # Errors
    ///
    /// Returns [`Error::NoSeparator`] when the colon is missing, [`Error::MalformedAlgorithm`] when
    /// the algorithm is not a token, and [`Error::MalformedValue`] when the digest value is not
    /// one, with `=` and `/` also allowed there per annotation #48.
    ...
    pub fn from_digest(algorithm: Algorithm, digest: &[u8]) -> Self {
        Self::from_digest_in(Format::recommended(algorithm), digest)
    }
    ...
    /// digest length is known, when it decodes to that many bytes. That length check is what leaves
    /// a known algorithm's digest fitting at most one encoding.
    #[must_use]
    ...
    ///
    /// An encoding whose digit count cannot give the expected length is ruled out before anything
    /// is decoded, so a known algorithm's value is normally decoded once.
...
mod tests {
    ...
    fn round_trips_a_digest_in_any_encoding(
        ...
        let labelled = LabelledDigest::from_digest_in(format, &digest);

        let decoded = labelled.decoded();

        prop_assert_eq!(labelled.algorithm(), Some(algorithm));
        prop_assert_eq!(labelled.encoding(), Some(encoding));
        prop_assert_eq!(decoded.as_deref(), Some(digest.as_slice()));
    }

    #[cfg(feature = "sha2")]
    #[test]
    fn a_hasher_finalizes_in_the_chosen_encoding() {
        let mut hasher = Algorithm::Sha256.hasher().expect("an enabled algorithm");
        hasher.update(b"hello");

        assert_eq!(
            hasher.finalize_labelled_in(Encoding::Base32).to_string(),
            "sha256:FTZE3OS7WCRQ4JXIHMVMLOPCTYNRMHS4D6TUEXTTAQZWFE4LTASA===="
        );
    }

    #[cfg(feature = "serde")]
    #[test]
    fn an_encoding_serializes_as_its_name() {
        assert_eq!(
            serde_json::to_string(&Encoding::Base32).expect("a JSON string"),
            "\"base32\""
        );
        assert_eq!(
            serde_json::from_str::<Encoding>("\"base16\"").expect("an encoding"),
            Encoding::Base16
        );
    }
    ...
    /// A value that decodes has the length its digit count tells, so the count never rules out an
    ...
    /// The archives this crate reads carry Base64 digests, in both of RFC 4648's alphabets and with
    /// the padding left off.
    #[test]
    ...
    /// Base64 spells different digests with the same letters in different cases, so a value read in
    /// it is not the same digest as the same letters in another case.
    #[test]
