...
// Each import is needed when its feature is the only one enabled, and is redundant when another
// crate re-exports the same trait, so this cannot be an expectation.
#[cfg(feature = "blake2")]
#[allow(
    unused_imports,
...
impl Algorithm {
    ...
    pub fn hasher(self) -> Option<Hasher> {
        let inner = match self {
            ...
            // `Xxh3::new` is const, and a build with only XXH3 would then be asked for a const
            // `hasher`.
            #[cfg(feature = "xxh3")]
...
mod tests {
    ...
    fn computes_a_digest_exactly_when_supported(
        #[strategy = prop::algorithm()] algorithm: Algorithm,
    ) {
        prop_assert_eq!(
