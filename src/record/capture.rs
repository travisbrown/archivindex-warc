...
impl<E: Extension> CaptureEvent<E> {
    ...
    /// Set the response or revisit record's `WARC-IP-Address`.
    #[must_use]
...
///
/// The response links to the request with `WARC-Concurrent-To`, and optional metadata links to the
/// response. These links point backward when records are written in the declared order; revisit and
/// warcinfo references may point outside the capture.
...
impl<E: Extension> CaptureRecords<E> {
    /// Return the response identity needed by [`revisit_exchange`](CaptureEvent::revisit_exchange).
    #[expect(
        clippy::missing_panics_doc,
