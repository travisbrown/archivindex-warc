...
impl Lifter {
    ...
    ///
    /// The grammar admits the union of the two versions' fields, so this is where a record is held
    /// to the version it declares. Rendering checks the same rule, since a record can also be
    /// assembled or edited after it is read.
    ...
    pub(super) fn take_uri(&mut self, field: Field) -> Option<Uri<String>> {
        match self.take_form(field)? {
            // Whether the URI was written in the angle brackets of the `"<" uri ">"` rule is the
            // grammar's record of how it arrived. It is written back as the declared version
            // requires rather than as it was read, so it is dropped here.
