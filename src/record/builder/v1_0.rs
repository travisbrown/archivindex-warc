...
//! record it builds as WARC 1.0. A `revisit` record has its own [`RevisitBuilder`], since a WARC
//! 1.0 record cannot carry the two fields WARC 1.1 named for that type.
//!
//! ```
...
///
/// It is [`super::RevisitBuilder`] without `WARC-Refers-To-Target-URI` and `WARC-Refers-To-Date`,
/// which WARC 1.1 named and a WARC 1.0 record cannot carry:
///
/// ```compile_fail
/// # use archivindex_warc::record::builder::v1_0;
...
/// # Errors
///
/// Returns [`Error::NotAUri`] if the target URI is not a URI, and [`Error::MalformedField`] when
/// the segment number is below `2`.
...
mod tests {
    ...
    /// Every entry point in [`v1_0`] declares WARC 1.0, which is the whole of what the module adds
    /// over the entry points on [`Record`].
    #[test]
    ...
    /// A `warcinfo` record built for WARC 1.0 says so in the body it opens with, since the fields
    /// that name the standard are the ones the file's version selects.
    #[test]
    ...
    /// A `revisit` record built for WARC 1.0 carries no field WARC 1.0 does not define, so it is
    /// written as a WARC 1.0 record without anything being refused.
    #[test]
    fn a_revisit_built_for_warc_1_0_is_written_as_warc_1_0() -> Result<(), BlockError> {
        ...
        // The builder declares the version, so the record is written as WARC 1.0 without anything
        // else being said.
        assert_eq!(record.version(), WarcVersion::V1_0);
