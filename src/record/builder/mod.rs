...
//!
//! For builders that accept an arbitrary content block, [`build`](ResponseBuilder::build) returns a
//! header and [`body`](ResponseBuilder::body) attaches the block and returns a record:
//!
//! ```
...
//!
//! Builders restrict which fields can be set, but some relationships are checked only during
//! semantic reading or rendering. For example, a revisit profile may require a payload digest.
//! Continuation segment numbers below `2` are rejected immediately.
//!
...
//! fields identifying the declared WARC version; a `metadata` body begins empty. Construct a record
//! directly if either type must carry another kind of block.
//!
//! [`digests`](ResponseBuilder::digests) computes a block digest and, when payload extraction
//! succeeds, a payload digest as the record is built. Existing digests are preserved.
//!
//! Records declare WARC 1.1. The [`v1_0`] module mirrors every entry point here for an archive that
//! has to be written as WARC 1.0.
//!
...
/// Initialize core fields with a generated identifier, capture date, and default content type.
...
/// Write what a `warcinfo` record says about the version of the file it opens, in the two fields
/// Annex B.1 of the standard writes it in.
...
macro_rules! body_setters {
    ($body:ty; $($setter:ident: $field:expr, $name:literal, $description:literal;)*) => {
        ...
        /// # Errors
        ///
        /// Returns [`fields::Error::UnwritableField`] when the name or value fails
        /// [`fields::Body::push`] validation.
        ...
        $(
            ...
            /// # Errors
            ///
            /// Returns [`fields::Error::UnwritableField`] when the value fails
            /// [`fields::Body::push`] validation.
...
macro_rules! shared_setters {
    ...
    // The version the record declares, which the header block says rather than a field of it. Only
    // the `v1_0` entry points declare anything but WARC 1.1, so this stays private.
    (v1_0, $($rest:tt)*) => {
    ...
    (digests, $($rest:tt)*) => {
        ...
        /// preserved, and a header built without a block gets none. The algorithm is chosen at the
        /// type level ([`Supported`](crate::value::Supported)), so an algorithm this build cannot
        /// compute is a compile error.
        #[must_use]
    ...
    (block, $($rest:tt)*) => {
        ...
        /// [`body`](Self::body) that reports [`BlockError::ContentLengthMismatch`] when the block
        /// is of another length. Building only a header stores the declaration without checking it
        /// against a block.
        #[must_use]
        ...
        /// `Content-Type`: the media type of the record's block, which for an archived HTTP message
        /// is `application/http` rather than the type that message declares.
        ///
    ...
    (payload, $($rest:tt)*) => {
        /// `WARC-Payload-Digest`: a digest over the record's payload, which need not be present in
        /// this record's block.
        #[must_use]
    ...
    (segment_origin, $($rest:tt)*) => {
        ...
        ///
        /// The value is always `1`, the only segment number allowed on an origin record.
        #[must_use]
...
macro_rules! builder_end {
    // The record types whose block is written as `warc-fields`, which the builder writes rather
    // than is given, so that building one finishes a record and cannot fail.
    ($builder:ident as $variant:ident, fields) => {
        impl<E: Extension> $builder<E> {
            /// Build a record from the configured header and body fields.
            ///
            /// A block these fields cannot express is written by constructing the record directly.
            #[must_use]
...
///
/// The setters write the record's `warc-fields` body, which starts by identifying the WARC version
/// of the file:
///
/// ```
...
impl<E: Extension> WarcinfoBuilder<E> {
    ...
    ///
    /// The body starts with `format` and `conformsTo` fields for the declared WARC version. Their
    /// setters replace these defaults.
    #[must_use]
...
impl<E: Extension> WarcinfoBuilder<E> {
    ...
    /// # Errors
    ///
    /// Returns [`fields::Error::UnwritableField`] if the combined value cannot be written as a body
    /// field.
    ...
    /// `WARC-Filename`: the name of the file holding this record, which no other record type may
    /// carry.
    ///
    /// # Errors
    ///
    /// Returns [`TextError`] for control characters other than tabs, or a malformed quoted
    /// filename.
...
impl<E: Extension> MetadataBuilder<E> {
    ...
    /// `hopsFromSeed`: the type of each hop from a starting seed URI to the archived one, which is
    /// the empty path for a seed itself.
    ///
...
impl<E: Extension> RevisitBuilder<E> {
    /// `WARC-Refers-To-Target-URI`: the target URI of the record this one revisits, which need not
    /// be this record's own. Named by WARC 1.1.
    #[must_use]
...
/// A builder for a `conversion` record, which holds transformed content from another record.
#[derive(Clone, Debug)]
...
impl<E: Extension> ContinuationBuilder<E> {
    ...
    /// # Errors
    ///
    /// Returns [`Error::NotAUri`] if the target URI is not a URI, and [`Error::MalformedField`]
    /// when the segment number is below `2`.
    pub fn new(
        target_uri: &str,
        date: impl Into<WarcDate>,
        segment_number: u64,
...
impl<E: Extension> ContinuationBuilder<E> {
    ...
    /// `WARC-Segment-Total-Length`: the length of every segment's block once reassembled, which the
    /// last `continuation` of a series carries and no other record does.
    #[must_use]
...
mod tests {
    ...
    /// Write a built record and read it back, which is the check that every field the builder set
    /// is a field the standard permits the record type and that the record says the same thing at
    /// either end.
    ...
    /// Each test record built with the available setters renders and parses back successfully.
    #[test]
    ...
    /// A record must carry an identifier, so a builder not told one names the record itself: each
    /// record built without one is built under a fresh `urn:uuid` name.
    #[test]
    ...
    /// A caller whose identifiers are its own says so, and the identifier it gives is the one the
    ...
    /// A `warcinfo` body starts by identifying its WARC version, and `build` writes those fields as
    /// the record block.
    #[test]
    ...
    /// A `metadata` body opens empty, so a record built from one holds only what its setters were
    /// told.
    #[test]
    ...
    /// A field the setters do not name is written by name, and each call adds a value rather than
    /// replacing the one before it, since a `warc-fields` body may repeat a field.
    #[test]
    ...
    fn a_record_type_with_a_customary_media_type_declares_it() -> Result<(), Error> {
        ...
        // Every other type holds whatever the record it was made from held, so its builder declares
        // nothing.
        assert_eq!(
            declared(
                Record::resource(TARGET_URI, date())
    ...
    /// Invalid filename values are rejected by the setter.
    #[test]
    ...
    /// A target URI is read where it is given, so a value that is not a URI is refused there rather
    /// than where the record is written.
    #[test]
    ...
    /// A vocabulary standing in for a small archiving extension, which defines one record type and
    /// adds nothing to the types the standard defines.
    ...
    /// A record under an extension is built the same way, through the record it is a record of, and
    /// one of a type the extension defines is written under the name that type gives itself.
    #[test]
