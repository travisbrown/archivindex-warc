...
//!
//! [`build`](ResponseBuilder::build) returns a header block for every record type but the two
//! described below, while [`body`](ResponseBuilder::body) attaches a content block and returns a
//! record:
//!
//! ```
...
macro_rules! body_setters {
    ($body:ty; $($setter:ident: $field:expr, $name:literal, $description:literal;)*) => {
        ...
        ///
        /// Each call adds another value, so a field written twice is written twice, unlike the
        /// setters here, which each own one field and replace the value it carries.
        ///
        /// # Errors
        ///
