...
mod tests {
    ...
    fn records() -> Result<Vec<Record>, Error> {
        Ok(vec![
            ...
            Record::response(TARGET_URI, date())
                ...
                .block_digest(added_digest(Algorithm::Sha256, RESPONSE_BLOCK.as_bytes()))
                .payload_digest(added_digest(Algorithm::Sha1, RESPONSE_PAYLOAD.as_bytes()))
    ...
    fn a_digest_the_caller_gave_outranks_the_digests_option() -> Result<(), BlockError> {
        let record: Record = Record::response(TARGET_URI, date())
            .expect("a well-formed target URI")
            .digests(marker::Sha1)
            .block_digest(added_digest(Algorithm::Sha256, b"hello"))
            .body("hello")?;

        assert_eq!(
            record.core().block_digest,
            Some(added_digest(Algorithm::Sha256, b"hello"))
        );
    ...
    fn a_builder_ended_with_build_builds_the_header_block_alone() -> Result<(), BlockError> {
        let block_digest = added_digest(Algorithm::Sha1, b"hello");
