...
pub(super) struct Renderer {
    ...
    ///
    /// Extension record types retain additional standard fields without type-specific restrictions.
    /// Version and repetition checks still apply.
    unconstrained: bool,
...
impl Renderer {
    ...
    /// Append a token-valued field, rejecting a token an extension spelled in a way the grammar
    /// does not admit.
    ...
    /// Append `WARC-Segment-Number` with the value `1`, the only value the standard permits on a
    /// record that is not a `continuation`, when the record is the origin of a series.
    ...
    /// header holds, so a name the standard defines is refused here rather than written beside the
    /// field it names.
    pub(super) fn push_as_read(&mut self, name: &str, value: &str) -> Result<(), RenderError> {
        ...
        // A value is held here with its folds already resolved, so a line break in one is a header
        // line the caller wrote into it rather than a fold, and would be read back as a field of
        // its own.
        ...
        // A value is written after the colon exactly as it is given, so one that does not open with
        // the linear white space the convention puts there is given a single space.
    ...
    /// the fields of its own type, so one written by the extension or kept as read can only be seen
    /// to repeat one of them at the end.
