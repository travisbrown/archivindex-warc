...
impl<F: Field> Body<F> {
    ...
    /// Replace all values of a field with one value.
    ///
...
fn check_writable<F: Field>(field: &F, value: &str) -> Result<(), Error> {
    ...
    // A value is held here with its folds already resolved, so a line break in one is a field line
    // the caller wrote into it rather than a fold.
...
mod tests {
    ...
    /// Setting a field gives it the value in place of every one it had, at the position its first
    /// appearance held, and adds it at the end when the body did not carry it at all.
    #[test]
    ...
    /// The length a body reports is the length of what it writes, whether that is the block it was
    /// read from or the canonical rendering it falls back to once it has been changed.
    #[test]
