...
//! - Serialization writes fields in serialization order, omits `None`, writes one line per sequence
//!   element, and validates each line with [`Body::push`].
//!
//! [`Body`] remains the lossless representation: it preserves unknown fields, repetition, order,
//! and the original bytes. Deserialization projects that data into a type; serialization creates a
//! new canonical body.
//!
//! ```
...
///
/// The type must serialize as a struct or map. Fields are written in the order they are serialized,
/// `None` fields are omitted, and a sequence writes one field line per element.
///
/// # Errors
///
/// Returns [`Error::Unsupported`] for an unsupported shape, or [`Error::Field`] when [`Body::push`]
/// rejects a name or value.
...
mod tests {
    ...
    /// Required, optional, repeated, and typed fields deserialize as expected, while unknown fields
    /// are ignored.
    #[test]
