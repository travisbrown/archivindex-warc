...
//!
//! Clause 5.19 of the WARC 1.1 standard requires this field to carry a type determined by examining
//! the payload, not one copied from a declaration in the block. [`payload_type`] identifies payload
//! bytes, and [`http_payload_type`] applies it to an archived HTTP message's entity-body.
//!
//! Identification is opt-in. A capture enables it with [`CaptureEvent::identify_payload_type`](
//! crate::record::capture::CaptureEvent::identify_payload_type), and a directly built record can
//! pass the result to
