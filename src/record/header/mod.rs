...
pub enum RevisitProfile {
    ...
    ///
    /// These rules apply to the standard profile URIs. The payload digest is required during both
    /// reading and rendering. The truncation declaration is checked only during rendering.
...
///
/// `WARC-Type` comes from the record variant, and the version comes from the containing header. The
/// type parameter supplies extension truncation reasons.
#[derive(Clone, Debug)]
pub struct CoreHeaders<E: Extension = NoExtension> {
    ...
    ///
    /// `None` has the record given the SHA-256 digest of its block when it is rendered. A declared
    /// digest under an algorithm this crate computes must be the digest of the block; one under
    /// any other algorithm is written as read and is not checked.
...
pub struct PayloadHeaders {
    /// `WARC-Payload-Digest`: `algorithm ":" value` over the payload, which need not be present in
    /// this record's block. A segmented record records it in its first segment, where it covers the
    /// payload of the reassembled logical record.
    pub payload_digest: Option<LabelledDigest>,
    /// `WARC-Identified-Payload-Type`: the media type of the payload as determined by inspecting
    /// it, never by promoting a declared `Content-Type` out of the block.
    pub identified_payload_type: Option<MediaType>,
}

/// The header of a `warcinfo` record, which describes the records that follow it up to the next
/// `warcinfo` record or the end of the file.
///
/// It describes a file rather than a capture, so it has no target URI, no IP address, and no
/// association with either a `warcinfo` record or a capture event. It has no payload, so it carries
/// no payload fields either.
...
pub struct WarcinfoHeader<E: Extension = NoExtension> {
    ...
    /// `WARC-Filename`: the name of the file holding this record. Permitted on no other record
    /// type.
    pub filename: Option<Text>,
    /// Whether `WARC-Segment-Number` is present with the value `1`, marking this record as the
    /// first segment of a series continued by `continuation` records.
    ///
...
/// The header of a `response` record, which holds a complete scheme-specific response including
/// protocol information. For `http` and `https` that is the full response message with its headers.
#[derive(Clone, Debug, Eq, PartialEq)]
...
/// The header of a `resource` record, which holds a resource without the protocol information a
/// `response` would carry. Its payload is its block.
#[derive(Clone, Debug, Eq, PartialEq)]
...
/// protocol information. For `http` and `https` that is the full request message with its headers.
#[derive(Clone, Debug, Eq, PartialEq)]
...
/// The header of a `metadata` record, which describes or accompanies another record in a way no
/// other record type covers.
///
/// It has no payload, so it carries no payload fields, and its target URI is optional: a metadata
/// record describes a record rather than a capture, and names what it describes with `refers_to`.
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct MetadataHeader<E: Extension = NoExtension> {
    ...
    /// The fields every record carries.
    pub core: CoreHeaders<E>,
    /// `WARC-Target-URI`: a copy of the target URI of the record this one describes. Optional here,
    /// unlike on the record types that describe a capture directly.
...
/// The header of a `revisit` record, which stands in for a `response` or `resource` whose content
/// was found to duplicate content already archived.
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct RevisitHeader<E: Extension = NoExtension> {
    ...
    /// identify the referenced record unambiguously.
    pub refers_to: Option<Uri<String>>,
    /// `WARC-Refers-To-Target-URI`: the target URI of the record this one revisits, which need not
    /// equal `target_uri`. Named by WARC 1.1; permitted on no other record type.
...
/// The header of a `conversion` record, which holds an alternative version of another record's
/// content produced by an archival process. Its payload is its block.
///
/// It is the product of a transformation rather than of a capture, so it carries neither an IP
/// address nor an association with a capture event.
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct ConversionHeader<E: Extension = NoExtension> {
    ...
    pub payload: PayloadHeaders,
    /// `WARC-Target-URI`: a copy of the target URI of the record that was converted. Mandatory.
    pub target_uri: Uri<String>,
    /// `WARC-Warcinfo-ID`: the `warcinfo` record describing this one.
    pub warcinfo_id: Option<Uri<String>>,
    /// `WARC-Refers-To`: the record whose content was converted. Recommended wherever it is
    /// practical to record it.
    pub refers_to: Option<Uri<String>>,
    /// Whether `WARC-Segment-Number` is present with the value `1`, marking this record as the
    /// first segment of a series continued by `continuation` records.
    pub segment_origin: bool,
...
///
/// Segment `1` is the origin record. This type accepts only values of `2` or greater for
/// continuations.
...
/// The header of a `continuation` record, which holds a later segment of a block too large for one
/// file.
///
/// Continuation headers forbid IP addresses, capture-event associations, and `WARC-Refers-To`.
/// Other optional fields are allowed, though the standard generally discourages them except for
/// block digests and final-segment length or truncation fields. Builders leave optional fields
/// unset unless supplied.
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct ContinuationHeader<E: Extension = NoExtension> {
    ...
    pub payload: PayloadHeaders,
    /// `WARC-Target-URI`: the target URI of the logical record, identical across every segment of
    /// the series. Mandatory.
    pub target_uri: Uri<String>,
    /// `WARC-Warcinfo-ID`: the `warcinfo` record describing this one.
    pub warcinfo_id: Option<Uri<String>>,
    /// `WARC-Segment-Number`: this segment's position in the series, counting from the origin
    /// record's `1`. Mandatory on a `continuation`.
    pub segment_number: SegmentNumber,
    /// `WARC-Segment-Origin-ID`: the record holding the first segment of the series. Mandatory on a
    /// `continuation` and permitted on no other record type.
    pub segment_origin_id: Uri<String>,
    /// `WARC-Segment-Total-Length`: the total length of every segment's block once reassembled.
    /// Present exactly on the last `continuation` of a series, so its presence is what marks the
    /// series as complete.
...
pub struct OtherHeader<E: Extension = NoExtension> {
    ...
    /// The fields every record carries.
    pub core: CoreHeaders<E>,
    /// Whether `WARC-Segment-Number` is present with the value `1`. Segmentation applies to any
    /// record type but `continuation`, extension types included.
    pub segment_origin: bool,
...
mod tests {
    ...
    /// Both versions' spellings of both standard profiles are recognized, and anything else is kept
    /// exactly as written.
    #[test]
    ...
    /// A profile URI names the version that defined it, which is not necessarily the version the
    /// record declares, so the two spellings are distinct values.
    #[test]
