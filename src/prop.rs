...
///
/// The content neither opens nor closes with white space, which a field value is read without, and
/// opens with a character that does not begin a `quoted-string`.
...
/// An absolute URI, with a path, a query, and a fragment that may hold characters RFC 3986 requires
/// be percent-encoded.
...
///
/// WARC 1.0 writes seconds and nothing else, so a value at any other precision is one that version
/// cannot round-trip.
...
///
/// Names are prefixed so that they cannot collide with a field the standard defines, which a record
/// of a standard type cannot carry beside the typed field of the same name.
...
/// An `application/warc-fields` block, built rather than parsed so that it carries no source block
/// of its own.
...
///
/// The identical payload digest profile asserts that the payload is elsewhere, so a record under it
/// carries a payload digest and, unless it declares the truncation, no block.
...
/// A `continuation` record with a segment number of at least `2`.
