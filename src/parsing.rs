...
/// `TEXT` accepts non-control bytes and horizontal tabs.
...
///
/// A fold is `CRLF` followed by at least one space or tab. Any other line break would end the field
/// line rather than continue the value, so a value holding one cannot be written back as it was
/// read. Control characters other than line breaks are left to the grammar layer.
...
///
/// Remove surrounding quotes and backslash escapes. Reject unescaped quotes, trailing backslashes,
/// and control characters other than tabs, whether escaped or not. The caller validates the decoded
/// bytes: media type parameters require UTF-8, while filenames may contain non-UTF-8 bytes.
///
/// # Errors
///
...
/// # Invariant
///
/// Every CR in `value` must begin a `CRLF SP` or `CRLF HTAB` fold. [`RecordHeader::parse`] and
/// [`RecordHeader::validate`] enforce this. Other callers must validate first: this helper assumes
/// each CR begins a fold and skips the following byte without checking it.
///
...
pub fn unfold(value: &[u8]) -> Cow<'_, [u8]> {
    ...
    while index < trimmed.len() {
        if trimmed[index] == b'\r' {
            // A CR opens a fold, per the invariant above, and the fold with the run of white space
            // after it stands for a single space.
            index += 2;
