...
/// ```
///
/// Filenames may contain non-UTF-8 bytes, so the content is stored as bytes. Control characters
/// other than tabs are rejected. RFC 2047 encoded words are preserved without decoding; see the
...
pub enum Error {
    /// The value holds a control character, which `TEXT` excludes unless it is linear white space.
...
impl Text {
    ...
    ///
    /// Only a quote and a backslash are escaped, so a value read with a redundant escape is written
    /// without it. [`Display`] spells the same value, but replaces any octet that is not valid
    /// UTF-8.
    #[must_use]
...
mod tests {
    ...
    fn parses_text_and_quoted_strings() {
        ...
        // Non-UTF-8 filename bytes are preserved when rendered.
