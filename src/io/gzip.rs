...
//! to be located and decompressed independently. [`MemberReader`] exposes the members as one stream
//! while retaining their file offsets, so a [`WarcReader`](crate::io::read::WarcReader) can place
//! records in the members holding them.
...
pub struct MemberReader<R> {
    ...
    chunk: u64,
    /// The file offset after the last completed member.
    ended_at: u64,
    /// The number of completed nonempty members.
    held: u64,
}
...
impl<R: BufRead> MemberReader<R> {
    ...
    /// Read the gzip members of `source` within `frame`, which `source` is positioned at the start
    /// of.
    ...
    ///
    /// Reaching that byte may mean opening the next member. A failure to do so is reported by the
    /// read that follows, and the location is then the end of what was read.
        ...
        loop {
            match std::mem::replace(&mut self.state, State::Done) {
                State::Between(mut source) => {
                    // The end of the stream is told from a member before a decoder is opened on it,
                    // since the decoder would report an incomplete header instead.
...
mod tests {
    ...
    /// Concatenate gzip members and return their end offsets.
    ...
    /// Read the decompressed contents and count nonempty members.
    ...
    /// All gzip members are exposed as one decompressed stream.
    #[test]
    ...
    /// Trailing non-gzip bytes produce an error.
    #[test]
