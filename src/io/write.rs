...
///
/// Compression is per record: a gzip writer stores every record as its own gzip member, so a member
/// boundary is always a record boundary and any record can be located and decompressed on its own.
/// This is the convention WARC 1.1 Annex B.1 describes, and the only framing this crate writes.
///
...
impl Compression {
    ...
    /// Write each record as an independent gzip member at [`DEFAULT_GZIP_COMPRESSION_LEVEL`].
    #[must_use]
...
///
/// Offsets count only bytes emitted by this writer's record-writing methods. Writing directly to
/// the inner writer does not advance the tracked position.
...
impl<W> WarcWriter<W> {
    ...
    ///
    /// The algorithm is chosen at the type level ([`Supported`]), so an algorithm this build cannot
    /// compute is a compile error. This digest is independent of the record's block and payload
    /// digests: it covers the bytes as stored, for indexes and containers that reference records by
    /// offset and length.
    #[must_use]
    ...
    ///
    /// This method does not flush or otherwise finish the inner writer. Use [`Self::flush`] first
    /// when necessary, or [`Self::finish`] when the inner writer is a [`std::io::BufWriter`].
    ///
    /// ```
...
impl<W: Write> WarcWriter<W> {
    ...
    ///
    /// Validation happens before writing, so an invalid record leaves the output unchanged. A sink
    /// error can leave a partial record; the tracked position includes any bytes the sink accepted.
    ///
    /// # Errors
    ///
    /// Returns [`Error::Sink`] when the output stream refuses the bytes, or [`Error::Raw`] carrying
    /// the reason the record cannot be written.
    ...
    ///
    /// The frame is taken whether or not the write succeeded, since a sink that failed partway has
    /// still consumed the bytes it accepted.
    fn tapped(
        &mut self,
...
impl WarcWriter<BufWriter<fs::File>> {
    ...
    ///
    /// Creates the file if needed and appends to its existing contents. The existing contents are
    /// not validated; use the same compression mode when appending. To replace a file, pass the
    /// result of [`std::fs::File::create`] to [`WarcWriter::new`].
    ///
...
mod write_tests {
    ...
    /// carrying a line break that would split the field in two, and a name that is not a token.
    #[test]
    ...
    /// A record whose declared length does not frame its body is refused, since it would take the
    /// following record's bytes with it when read back.
    #[test]
...
impl WarcWriter<BufWriter<fs::File>> {
    ...
    ///
    /// Each record is stored as an independent gzip member at [`DEFAULT_GZIP_COMPRESSION_LEVEL`],
    /// so reported offsets frame the compressed member and locate the record in the file. Use
    /// [`Self::with_compression`] to choose another level.
    ///
