...
//! [`Placement`]s. Seekable readers support indexed access through the `*_record_at` methods. Use
//! an iterator's `records` method to discard locations.
...
pub enum Error {
    ...
    HeaderBlockTooLarge,
    /// The record's declared `Content-Length` is too large for its body to be buffered in memory on
    /// this platform.
    ...
    UnexpectedEndOfBody,
    /// The `\r\n\r\n` terminator after the record's body was missing or malformed. The record was
    /// read completely, but is invalid.
    ...
    ///
    /// This includes a record declaring a version this crate does not read. Its header block is not
    /// parsed, so its body cannot be skipped, and iteration ends at that record.
    #[error(transparent)]
    Raw(#[from] raw::Error),
...
///
/// For plain input, a frame covers the record from its version line through its terminator. For
/// gzip input, it covers complete compressed members containing only that record and optional
/// blank-line padding. [`Location::frame`] returns `None` when records share a member.
#[derive(Clone, Copy, Debug, Default, Eq, PartialEq)]
pub struct Frame {
    /// The record's byte offset, or the first gzip member's offset for compressed input.
    pub offset: u64,
    /// The byte length of the record, or of its complete gzip members for compressed input.
    pub length: u64,
}

/// Where a record lies in the members of the gzip stream it was read from.
///
/// The members holding a record locate it in the compressed file, as an index over the file needs,
/// when they hold nothing else; [`Location::frame`] is that location. Blank lines before or after
/// the record within its members are read past as padding, and count as nothing.
...
pub struct Placement {
    ...
    /// record, or the blank lines after it, when the record ends that member. A record followed in
    /// its last member by another has no length that locates it.
...
impl Placement {
    /// Where a record lies, given the location of its first octet and of the octet after its last.
...
impl Location {
    ...
    ///
    /// Every record in a plain stream has one. A record in a gzip stream has one when the members
    /// holding it hold nothing else, blank lines aside: the frame covers those members, and
    /// decompressing the octets it covers yields the record alone.
    #[must_use]
...
/// A record value and its input location.
///
...
impl<T> Located<T> {
    ...
    /// Map the value while preserving its location and padding count.
...
///
/// Created by an iterator's `records` method. Its `Result` items can be collected into a
/// `Result<Vec<_>, _>`.
...
macro_rules! located_iterator {
    ($iterator:ident<$($param:ident),+>, $($reading:ident).+) => {
        impl<$($param),+> $iterator<$($param),+> {
            ...
            ///
            /// The location is empty and stands where the input ends; the blank lines are the ones
            /// the input ends with. This is `None` until the iterator has returned `None`, and
            /// after an error, since where the reader stopped is then unknown.
            #[must_use]
...
pub(crate) struct Position {
    /// The offset in the file of the member holding the byte, or the end of the file after the last
    /// member.
...
impl<R: BufRead> WarcReader<R> {
    ...
    ///
    /// The stream ends with its last member; trailing data that is not another member is an error.
    /// Each record's [`Placement`] locates it in the compressed stream:
    ///
    /// ```
    ...
    ///
    /// Each record is a [`raw::Record`]: its field names and values are exactly the ones on the
    /// wire, checked only for the grammar of a header block and for the `Content-Length` that
    /// frames the body.
    ...
    ///
    /// The predicate is shown a [`record::RecordHeader<E>`](record::RecordHeader): the header block
    /// checked against the rules for its record type and its declared version. The body of a
    /// refused record is consumed without being buffered.
    ///
    /// `E` is the extension in force, as for [`iter_records`](Self::iter_records). It must be named
    /// at the call site, along with a `_` for the predicate's type: `filter_records::<NoExtension,
    /// _>(..)`.
...
impl<R: BufRead + Seek> WarcReader<R> {
    ...
    ///
    /// The frame is one an index holds for the record: the one an iterator yielded the record with,
    /// as [`Location::frame`] reports it, or the one the writer returned. Only the octets the frame
    /// covers are read, so the reader can read any number of records in any order:
    ///
    /// ```
    /// use std::io::Cursor;
    ///
    ...
    /// # Errors
    ///
    /// A frame holding no record is [`Error::EmptyFrame`], and one holding a further record after
    /// the first and the blank lines after it is [`Error::OverfullFrame`]. A frame that ends within
    /// its record fails as a stream that ends there does.
    ...
    /// # Errors
    ///
    /// As for [`untyped_record_at`](Self::untyped_record_at), and [`Error::Record`] when the record
    /// breaks a rule of its type or its declared version.
...
///
/// Unlike [`BufRead::read_until`], which buffers an entire delimiter-free stream within a single
/// call, this never reads more than `limit - buffer.len()` bytes.
...
fn read_body<R: BufRead>(reader: &mut R, expected_body_len: u64) -> Result<Vec<u8>, Error> {
    // The body plus its 4-byte terminator must fit in a single in-memory buffer. A length for which
    // that is impossible (a hostile value near the platform maximum, or a >4 GiB record on a 32-bit
    // target) is rejected up front, rather than overflowing the arithmetic below.
    ...
    // `Content-Length` cannot force a huge up-front allocation. Reads are bounded by the declared
    // length: exactly the body and its 4-byte `\r\n\r\n` terminator are consumed, regardless of
    ...
    // A record whose actual body outruns its declared length puts body bytes where the terminator
    // belongs, so overlong records surface here too.
...
/// buffering any of it.
///
/// The terminator is checked exactly as [`read_body`] checks it, so a record whose body outruns its
/// declared length is an error whether or not the body was wanted. Nothing is allocated for the
/// body, so even a length too large to buffer can be skipped.
fn skip_body<R: BufRead>(reader: &mut R, expected_body_len: u64) -> Result<(), Error> {
    ...
    while remaining > 0 {
        ...
        // `remaining` may exceed what a `usize` can hold, so the minimum is taken in the wider type
        // before narrowing.
        let taken = available
            .len()
...
struct Reading<R> {
    ...
    /// Set once the input has ended or an error has been yielded. Every error here is stream-level:
    /// it leaves the reader at an unspecified position, so iteration fuses rather than yielding
    /// garbage records from the middle of a partly consumed one.
    finished: bool,
...
impl<R> Reading<R> {
    /// The frame of the header block read last and everything consumed after it, through the end of
    /// the record or of the read that failed.
...
impl<R: BufRead> Reading<R> {
    ...
    /// buffer.
    ///
    /// Skip and count extra blank lines before the next version line. Some archives include this
    /// padding even though it is outside the record grammar.
    ///
    ...
    /// Read the rest of a header block whose first line the read `first` put in the header buffer,
    /// up to and including the blank line that terminates it, reading at most [`MAX_HEADER_BLOCK`]
    /// bytes in all.
    ///
    ...
    ///
    /// Returns `None` at a clean end of stream, and thereafter. The body must be read or skipped
    ...
    fn next_header(&mut self) -> Option<Result<(raw::RecordHeader, u64), Error>> {
        ...
        // Whatever the header buffer holds was consumed after the padding before it, and begins the
        // record. At the end of the input it holds nothing.
    ...
    /// End the record read last where reading stopped, placing it in the members of a gzip stream.
    ///
    ...
    ///
    /// Conversion failures consume the body before being returned. A stream failure while consuming
    /// that body takes precedence, since it prevents reading further records.
...
///
/// I/O and framing errors stop iteration; all later calls return `None`. Unsupported WARC versions
/// are framing errors because their headers are not parsed, so their bodies cannot be skipped.
...
/// grammar is a record-level error: the record is consumed completely, so iteration continues with
/// the next one.
...
///
/// `E` is the extension in force. A record the standard does not permit is a record-level error, so
/// iteration continues with the next one; the iterator fuses only where [`UntypedIter`] does.
pub struct RecordIter<R, E = NoExtension> {
    untyped: UntypedIter<R>,
    /// The extension is carried as a marker rather than a value. The function type keeps `E` from
    /// affecting the iterator's auto traits.
...
/// header block that fails to parse is a record-level error: the body is consumed before the error
/// is yielded, leaving the reader at the next record.
...
mod iter_raw_tests {
    ...
    /// A stream-level error leaves the reader at an unspecified position, so the iterator fuses
    /// instead of yielding garbage parsed from the middle of the broken record.
    #[test]
    ...
    /// A record whose actual body outruns its declared `Content-Length` puts body bytes where the
    /// terminator belongs; only the declared range is read.
    #[test]
    ...
    /// Every field line is kept as it was written: the name in its own spelling, the value with the
    /// white space around it, and all of them in the order they appeared.
    #[test]
    fn basic_record() {
        let raw = b"\
            WARC/1.0\r\n\
    ...
    /// A field line that is not `field-name ":" field-value` is rejected with an error carrying
    /// that line, rather than it (and every line after it) being silently dropped.
    #[test]
    ...
    /// A field whose value does not match the grammar its name selects is a record-level error: the
    /// record is consumed completely, so reading continues with the next one.
    #[test]
    ...
    /// A record without `Content-Length` cannot be framed; it is rejected with an error naming the
    /// missing field rather than misread as having an empty body.
    #[test]
    ...
    /// A hostile `Content-Length` near the unsigned 64-bit maximum must be rejected cleanly: the
    /// buffered path cannot possibly hold such a body, and the length arithmetic must not overflow
    /// (which previously panicked in debug builds and wrapped in release).
    #[test]
    ...
    /// A `Content-Length` value beyond the unsigned 64-bit range is not a length at all, so the
    ...
    /// requires, and must be reported as an error rather than reading as an empty archive.
    #[test]
...
mod filter_tests {
    ...
    /// A record the predicate refuses is framed exactly as a kept one is, so a body that outruns
    /// its declared length is an error whether or not the body was wanted.
    #[test]
    ...
    /// Skipping must report truncation even if header conversion also failed, and must retain the
    /// consumed frame before fusing the iterator.
    #[test]
    ...
    /// A header block the grammar refuses is a record-level error: its body is consumed before the
    /// error is yielded, so the record after it is still read.
    #[test]
...
mod gzip_tests {
    ...
    /// A file whose records were each compressed as an independent member reads as a whole archive.
    #[test]
    ...
    /// The frames of a gzip file's records are the members the writer stored them in, and stand at
    /// the end of the file once it is read.
    #[test]
    ...
    /// The `filter` iterators consume skipped bodies without buffering them and report the same
    /// placements.
    #[test]
    ...
    /// A frame a placement reports reads back as its record, whichever members hold it, and a frame
    /// cut within a member is an error.
    #[test]
...
mod frame_tests {
    ...
    /// The records of `archive` with their frames, and the frame reported once reading has ended.
    ...
    /// Padding before a record is not part of its frame, and padding after the last record moves
    ...
    /// A record is framed at every level, whether it reads, fails to lift, or is kept by a filter,
    /// and a filter that refuses every record still reaches the end.
    #[test]
...
mod access_tests {
    ...
    /// A WARC 1.1 resource record framed by the length of its body, with the fields the semantic
    /// level requires.
    fn record(body: &str) -> String {
        format!(
            "WARC/1.1\r\n\
    ...
    /// The untyped and semantic levels lift the record the frame locates, and report what lifting
    /// refuses.
    #[test]
