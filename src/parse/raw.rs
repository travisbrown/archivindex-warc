...
//!
//! This is the crate's lowest representation level. A [`Record`] contains a declared version, field
//! lines, and a fully buffered body. Field names preserve their case, and values preserve all white
//! space and folded continuations.
//!
...
pub enum Error {
    ...
    ///
    /// The rest of the header block is not parsed, so a reader cannot frame the record's body and
    /// stops at it.
    #[error(transparent)]
    ...
    /// The record contains more than one `Content-Length` field.
    ...
    /// A record's declared `Content-Length` does not match the body it carries, so writing it would
    /// produce an archive that cannot be read back.
...
/// colon and the CRLF ending the field, including leading and trailing white space and the `CRLF
/// SP` sequences of any folded continuation lines. A header block read from an archive writes back
/// byte for byte.
...
impl RecordHeader {
    ...
    /// # Errors
    ///
    /// Returns [`Error::MalformedVersionLine`] when the block does not begin with a `WARC/` version
    /// line and [`Error::MalformedVersion`] when that line names a version this crate does not
    /// support, [`Error::MalformedFieldLine`] when a line is not `field-name ":" field-value` or
    /// holds a line break that does not fold the value, [`Error::UnexpectedEndOfHeaderBlock`] when
    /// the block does not end with its blank line, and [`Error::MissingContentLength`],
    ...
    pub fn parse(block: &[u8]) -> Result<(Self, u64), Error> {
        ...
        loop {
            ...
            // beginning with linear white space (a continuation with nothing to continue) and white
            // space between the name and the colon (which a `warc-fields` body tolerates but the
            // header block grammar does not allow).
            ...
            // The name is a token, which admits only ASCII, so the lossy conversion never actually
            // replaces anything.
            let name = String::from_utf8_lossy(name).into_owned();
            let value = &block[value_start..value_end];

            // A CR that opens no fold would have ended the field line, and every layer above reads
            // one in a value as a fold, so it is refused here rather than carried along.
    ...
    ///
    /// Validation failures leave the output unchanged. An I/O failure may leave partial output.
    ///
    /// # Errors
    ///
    /// Returns [`write::Error::Raw`](crate::io::write::Error::Raw) carrying the validation failure
    /// that stopped the write, or [`write::Error::Sink`](crate::io::write::Error::Sink) carrying a
    /// failure of the writer itself.
...
///
/// A record assembled directly may have invalid field lines or a `Content-Length` that differs from
/// its body length. [`validate`](Record::validate) checks these conditions before writing.
...
impl Record {
    ...
    ///
    /// Returns [`Error::MalformedFieldLine`], [`Error::MissingContentLength`],
    /// [`Error::RepeatedContentLength`], [`Error::MalformedContentLength`], or
...
mod tests {
    ...
    /// The value is every byte after the colon, so the white space surrounding it is kept rather
    /// than trimmed away.
    #[test]
    ...
    /// A line naming a version is reported as the version it names, so the fault a caller reads is
    /// the version number alone. A line naming none is reported as the line, since there is no
    /// version in it to report.
    #[test]
    ...
    /// The only line break a value may hold is the CRLF of a fold, and every layer above reads a CR
    /// in a value as opening one, so a bare CR must not get past this layer.
    #[test]
    ...
    /// Every fault the octets can have says what it is, and none of them wraps another error for a
    /// caller to unwrap: the version error a malformed version line carries is reported as itself,
    /// since the variant is transparent.
    #[test]
