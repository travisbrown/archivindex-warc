...
//!
//! This representation sits between [`crate::parse::raw::Record`] and [`crate::record::Record`]. It
//! uses the union of the WARC 1.0 and 1.1 grammars, but does not check values or repeated fields
//! against the declared version and record type. Values are presented as they are spelled: the
//! encoded words of RFC 2047 are not decoded, following community recommendation #67 of the
//! annotated WARC 1.1 standard, as the [crate documentation](crate) describes.
...
impl RecordHeader {
    ...
    ///
    /// This conversion preserves names and value bytes without further validation. The raw writer
    /// validates the header before writing it.
    #[must_use]
...
mod tests {
    ...
    /// The error identifies the field and exposes its value-grammar error as the source.
    #[test]
