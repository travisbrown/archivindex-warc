...
impl Field {
    ...
    /// The field this name denotes, compared case-insensitively, or `None` for a name no version of
    /// the standard defines.
    #[must_use]
    pub fn from_name(name: &str) -> Option<Self> {
        // Filter candidates by length before comparing case-insensitively. Tests check these
        // lengths against the standard spellings.
...
impl HeaderName {
    ...
    ///
    /// An extension name is left alone, since the spelling it was read with is the only one it has.
...
mod tests {
    ...
    fn serde_round_trip() {
        ...
        // The derive encodes the variant rather than either spelling of the name, so a field is
        // written the same way whatever an archive spells it.
        assert_eq!(
