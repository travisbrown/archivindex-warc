...
///
/// Variants represent grammar rules, which several fields can share. This type accepts the union of
/// the WARC 1.0 and 1.1 grammars. The semantic representation checks the declared version.
#[derive(Clone, Debug, Eq, PartialEq)]
pub enum ValueForm {
    /// A URI, and whether it was written inside the angle brackets of the `"<" uri ">"` rule.
    ///
    /// WARC 1.0 brackets every URI-valued field; WARC 1.1 brackets only the five fields containing
    /// record identifiers. Both forms parse here. `bracketed` records the source form; semantic
    /// rendering uses the brackets required by the declared version.
    Uri {
...
impl ValueForm {
    ...
    fn write_to(&self, out: &mut Vec<u8>) {
        match self {
            // Text may contain non-UTF-8 bytes, so render it directly instead of using its lossy
...
impl std::fmt::Display for ValueForm {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            // A date renders at its own precision, which is the WARC 1.1 spelling. Rendering it for
            // a record's version is the semantic level's business.
...
/// included, to support byte-exact round-tripping. The form is parsed from the value's content: the
/// bytes with folds resolved to single spaces and surrounding white space removed, as `field-value
/// = *( field-content | LWS )` prescribes.
#[derive(Clone, Debug, Eq, PartialEq)]
...
impl From<ValueForm> for HeaderValue {
    ...
    ///
    /// The rendered value begins with a single space, following the convention for spacing after a
...
mod tests {
    ...
    /// Bytes that are not UTF-8 fail every grammar that reads a value as text, and are kept where
    /// the grammar is `TEXT`, which admits any octet, or where there is no grammar.
    #[test]
