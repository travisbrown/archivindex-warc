...
- `archivindex record` checks each record against the standard's rules for its type and declared
  version. It also checks declared block and payload digests when their algorithms are supported
  and the content is available. Payload digests on segmented or truncated records are not checked.
