...
/// Detect inputs known to make the warcat 0.3.4 decoder loop indefinitely.
///
/// Its decoder loops forever on a CR that no LF follows anywhere in a header block, on input that
/// ends inside a record (in a header line, in the body, or in the four bytes of the record
...
/// Walk the records of `input` as the decoder frames them, stopping with `None` at the first thing
/// the decoder would reject rather than loop on.
///
/// A record's body is framed by its last `Content-Length`, whose value the decoder accepts only as
/// digits after a single space; a line is accepted with or without a CR before its LF.
/// `member_ends` holds the ascending offsets at which gzip members end, and is empty for input that
/// is not gzip.
